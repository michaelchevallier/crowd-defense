...
namespace CrowdDefense.Systems
{
    ...
    public class WaveManager : MonoSingleton<WaveManager>
    {
        ...
        private void SpawnEnemy(EnemyType type, int wavePortalIdx, EnemyVariant variant = EnemyVariant.Normal)
        {
            ...
            bool skipTelegraph = SaveSystem.IsHardcoreRun
                || (levelData!.IsEndless || LevelRunner.Instance?.IsEndlessRun == true);
            ...
            {
                StartCoroutine(SpawnWithTelegraph(type, spawnPos, resolvedPathIdx, variant));
            }
        }

        private static readonly Color _telegraphColor     = new Color(1f, 0.2f, 0.1f, 0.6f);
        private static readonly Color _telegraphColorBoss = new Color(1f, 0.05f, 0f, 0.85f);
        private const int   _telegraphSegments   = 64;
        private const float _telegraphDuration   = 1.0f;
        private const float _telegraphFadeStart  = 0.8f;  // seconds before spawn: begin fade-out
        private const float _telegraphPulseHz    = 4f;
        private const float _telegraphAlphaMin   = 0.4f;
        private const float _telegraphAlphaMax   = 0.8f;
        private const float _telegraphRadiusNorm = 0.6f;
        private const float _telegraphRadiusBoss = 1.5f;

        private System.Collections.IEnumerator SpawnWithTelegraph(
            EnemyType type, Vector3 spawnPos, int pathIdx, EnemyVariant variant)
        {
            bool isBoss = type.IsBoss;
            float radius = isBoss ? _telegraphRadiusBoss : _telegraphRadiusNorm;
            Color baseColor = isBoss ? _telegraphColorBoss : _telegraphColor;

            // Boss telegraph audio before the indicator appears
            if (isBoss)
                AudioController.Instance?.Play("boss_telegraph_horn", 1.2f);

            // Build the ring mesh once — reuse vertices[], no per-frame alloc
            var telegraphGO = new GameObject("SpawnTelegraph");
            telegraphGO.transform.position = spawnPos + Vector3.up * 0.02f; // just above ground
            var meshFilter   = telegraphGO.AddComponent<MeshFilter>();
            var meshRenderer = telegraphGO.AddComponent<MeshRenderer>();
            ...
            // Build ring mesh (annulus: inner 85% outer 100%)
            int seg = _telegraphSegments;
            var vertices  = new Vector3[seg * 2];
            var triangles = new int[seg * 6];
            var colors32  = new Color32[seg * 2];

            float innerR = radius * 0.85f;
            float angleStep = 2f * Mathf.PI / seg;
            for (int i = 0; i < seg; i++)
            {
                float angle = i * angleStep;
                float cos = Mathf.Cos(angle);
                float sin = Mathf.Sin(angle);
                vertices[i]       = new Vector3(cos * innerR, 0f, sin * innerR);
                vertices[seg + i] = new Vector3(cos * radius, 0f, sin * radius);
                int tri = i * 6;
                int next = (i + 1) % seg;
                triangles[tri]     = i;
                triangles[tri + 1] = seg + i;
                triangles[tri + 2] = next;
                triangles[tri + 3] = next;
                triangles[tri + 4] = seg + i;
                triangles[tri + 5] = seg + next;
            }
            ...
            while (t < _telegraphDuration)
            {
                ...
                float alpha;
                if (t >= _telegraphFadeStart)
                {
                    // Fade out in the last 0.2s
                    float fadeT = (t - _telegraphFadeStart) / (_telegraphDuration - _telegraphFadeStart);
                    alpha = Mathf.Lerp(baseColor.a, 0f, fadeT);
                }
                else
                {
                    // Pulse alpha between min and max at telegraphPulseHz
                    float pulse = Mathf.Sin(t * _telegraphPulseHz * 2f * Mathf.PI) * 0.5f + 0.5f;
                    alpha = Mathf.Lerp(_telegraphAlphaMin, _telegraphAlphaMax, pulse);
                }

                mat.color = new Color(baseColor.r, baseColor.g, baseColor.b, alpha);

                // Flash at 0.8s boundary
                if (t >= _telegraphFadeStart && t < _telegraphFadeStart + Time.deltaTime * 2f)
                    mat.color = new Color(1f, 1f, 1f, 0.9f);

                yield return null;
            }

            // Actual spawn after telegraph animation completes
            DoSpawnEnemy(type, spawnPos, pathIdx, variant);

            UnityEngine.Object.Destroy(telegraphGO);
